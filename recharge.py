...
def itemizeRockimagerLog(df_RockImager_1, df_RockImager_2, start_date, end_date, costRockImagerTime):
    ...
    if len(df_RockImager_1.loc[df_RockImager_1['Group']=='nan'])>0:
        print('df_RockImager_1 has nan in Group <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
    if len(df_RockImager_2.loc[df_RockImager_2['Group']=='nan'])>0:
        print('df_RockImager_2 has nan in Group <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
...
def itemizeFacilityFees(allUsers, coreUsers, coreFacilityFee, associateUsers, assocFacilityFee, industryUsers, industryFee, start_date, end_date):
    users = []
    usersFee = []
    ...
    df_facFee = df_facFee.groupby([pd.Grouper(freq="M"), 'Group']).sum(numeric_only=True)
    return df_facFee


def findRechargeConstants(df_rechargeConst):
    def findRechargeConst(rowSelector, colSelector):
        return np.float(str(df_rechargeConst.loc[df_rechargeConst['Item'] == rowSelector][colSelector] \
                        .values[0]).replace('$', '', ))

    def findRechargeStr(rowSelector, colSelector):
        return str(df_rechargeConst.loc[df_rechargeConst['Item'] == rowSelector][colSelector] \
                        .values[0]).replace('$', '', )
    ...
    facilitiesAndAdministrationRate = findRechargeConst('facilities and administration rate', 'Price')
    facilitiesAndAdministrationRateDate = findRechargeStr('facilities and administration rate', 'Description')

    daysUntilExpired = 365
    expiredFinancialInfo = False
    if datetime.now() >= datetime.strptime(workingCapitalDate,  "%m/%d/%Y") + timedelta(days=daysUntilExpired):
        print('workingCapital value expired. Please update financial information on Google Sheets')
        expiredFinancialInfo = True


    if datetime.now() >= datetime.strptime(priorYearBalanceDate,  "%m/%d/%Y") + timedelta(days=daysUntilExpired):
        print('priorYearBalance value expired. Please update financial information on Google Sheets')
        expiredFinancialInfo = True

    if datetime.now() >= datetime.strptime(totalLabPersonnelDate,  "%m/%d/%Y") + timedelta(days=daysUntilExpired):
        print('totalLabPersonnel value expired. Please update financial information on Google Sheets')
        expiredFinancialInfo = True

    if datetime.now() >= datetime.strptime(facilitiesAndAdministrationRateDate,  "%m/%d/%Y") + timedelta(days=daysUntilExpired):
        print('facilitiesAndAdministrationRate value expired. Please update financial information on Google Sheets')
        expiredFinancialInfo = True
    if expiredFinancialInfo:
        exit(1)

    return [coreMultl, coreFacilityFee, assocMult, assocFacilityFee, regMult, regFacilityFee, indMult, indFacilityFee, costHDP, costSDP,
            costMosqTip, costMosquitoTime, costDragonflyTime, costRockImagerTime, costMXOne, costHDSConsume,
            costSDSConsume, costDFlyReservoir, costDFlyTip], [workingCapital, priorYearBalance, totalLabPersonnel, facilitiesAndAdministrationRate]

def calculateIndustryRecharge(industryUsers, externalRateConstants, outSummary, date_range):
    workingCapital, priorYearBalance, totalLabPersonnel, facilitiesAndAdministrationRate = externalRateConstants
    volume = 1
    unitLabPersonnel = 1
    ...
    outSummary.loc[(slice(None),industryUsers), "Total Charge"] = \
        (((
            (
                ...
                outSummary.loc[(slice(None),industryUsers), "Mosquito Total Cost"]+ \
                outSummary.loc[(slice(None),industryUsers), "RockImager Total Cost"]+ \
            ...
            workingCapital/12+priorYearBalance/12) * unitLabPersonnel/totalLabPersonnel + \
        ...
        facilitiesAndAdministrationRate) * volume
    ...
    outSummary.loc[(slice(None),industryUsers), "totalLabPersonnel"] = totalLabPersonnel
    outSummary.loc[(slice(None),industryUsers), "facilitiesAndAdministrationRate"] = facilitiesAndAdministrationRate
...
def calculateRecharge(dfs, date_range, users):
    ...
    coreMultl, coreFacilityFee, assocMult, assocFacilityFee, regMult, regFacilityFee, indMult, indFacilityFee, costHDP, costSDP, \
    costMosqTip, costMosquitoTime, costDragonflyTime, costRockImagerTime, costMXOne, costHDSConsume, costSDSConsume, \
    costDFlyReservoir, costDFlyTip = findRechargeConstants(df_rechargeConst)[0]
    ...
    df_facFee = itemizeFacilityFees(allUsers, coreUsers, coreFacilityFee, associateUsers, assocFacilityFee, industryUsers, indFacilityFee, start_date,
                                    end_date)
    ...
    df_GL_largePayment = df_GL[df_GL['Recharge Category'] == 'largePayment'].groupby(
        [pd.Grouper(freq="M")]).sum().loc[start_date:end_date]
    ...
    totalAvgUsageProportion=sum(list(pd.merge(monthlyRechargeTotal, df_queriedRechargeByGroup[['Group','avg Usage prop']], how='left', on='Group')['avg Usage prop']))
    monthlyRechargeTotal.loc[((monthlyRechargeTotal.index.levels[0]), 'avg Usage prop')] = [n/totalAvgUsageProportion for n in list(pd.merge(monthlyRechargeTotal, df_queriedRechargeByGroup[['Group','avg Usage prop']], how='left', on='Group')['avg Usage prop'])]
    ...
    for index, row in monthlyRechargeTotal.iterrows():
        if (index[0] in df_GL_largePayment.index):
            lst_largeExpenses.append(row['avg Usage prop'] * df_GL_largePayment.loc[index[0]]['Actual'])
        else:
    ...
    outSummary = monthlyRechargeTotal[
        ['Facility Fee', 'Screens Total Cost', 'Mosquito Total Cost', 'RockImager Total Cost', 'DFly Total Cost',
         'Raw Usage', 'Usage prop', 'Month Dist. Cost', 'Large Expense Cost', 'Payroll Cost', 'Use Multiplier', 'Total Charge']]
    outSummary = calculateIndustryRecharge(industryUsers=industryUsers,externalRateConstants=findRechargeConstants(df_rechargeConst)[1], outSummary=outSummary, date_range=date_range)
