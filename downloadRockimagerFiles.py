...
def file_matches_date(str1, str2, acceptable_wrong):
    ...
    if (matches >= len(str1)-1-acceptable_wrong):
        print(temp_str1,temp_str2, True)
...
def main():
    ...
    gdrive_room_list = drive.ListFile({'q': "'1Bk8D2ZkbpX3kkJHLM1lduPTD_RbCiXDN' in parents and trashed=false"}).GetList()
    ...
    gdrive_cold_list = drive.ListFile({'q': "'1U1a6RnhTr37n3BUFEmA4K4j7D6titeMv' in parents and trashed=false"}).GetList()
    ...
    subprocess.run(["cp",cold_local,"/Users/liam_msg/Documents/automate_MSG_automate_recharge/rockimagerBacklogs/"])
    subprocess.run(["cp",room_local,"/Users/liam_msg/Documents/automate_MSG_automate_recharge/rockimagerBacklogs/"])
